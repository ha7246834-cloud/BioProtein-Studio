...
The new module integrates the workflow that is often carried out manually with GSDS, NCBI CD-Search, MEME Suite and TBtools.
...
- CDS translation QC
- Gene structure from:
  - matching CDS + genomic FASTA using **EMBOSS est2genome**, or
  - an NCBI reference CDS feature when a protein accession can be resolved
...
This installs the Streamlit application and remote CDD support, but the automatic MEME and spliced est2genome steps will remain unavailable until their executables are installed.
...
All three should use the same FASTA identifiers. The application translates/validates sequence data, maps CDS to genomic DNA, runs CDD and MEME, applies QC and creates final figures.
...
CDD and MEME can run automatically. Gene structure cannot be reconstructed scientifically from protein sequence alone, so the user must supply matching CDS + genomic sequence or a valid reference annotation/accession.

## Output package

The one-click download contains:

- input FASTA files
- sequence QC
- gene structure coordinates and QC
- raw est2genome text
- raw and non-redundant CDD hits
- CDD Search-ID and raw output
- MEME XML, original MEME result ZIP and logos
- motif coordinates and QC
- integrated publication figures
- `run_manifest.json` containing analysis parameters
- methods/QC notes
