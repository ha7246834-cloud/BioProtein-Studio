...
with col1:
    ...
    st.write(
        "Validate raw or multi-FASTA proteins and calculate physicochemical properties "
        "with statistical summaries and publication outputs."
    )
with col2:
    st.subheader("🧬 Gene Structure, Domains, Motifs & Phylogeny")
    st.write(
        "Automatic Protein + Species reference resolution, annotation-reconciled exon/intron mapping, NCBI Batch CDD conserved-domain search, "
        "MEME motif discovery, tree-ordered visualization, and scientific validation."
    )
with col3:
    st.subheader("📊 Reproducible Research Outputs")
    st.write(
        "Export CSV/Excel data, raw engine outputs, validation tables, run manifests, "
        "and 600-DPI PNG/TIFF plus SVG/PDF figures."
    )
...
2. Open **Gene Structure, Domains, Motifs & Phylogeny** for the genome-wide family workflow.
3. Provide protein FASTA. For automatic gene structure, enter a species/taxon; the app can resolve an annotated NCBI reference genome and reconcile miniprot mappings against the reference GFF3.
4. Matching CDS + genomic FASTA, GFF3/GTF, or real NCBI accessions remain supported as validation/reference routes.
...
st.warning(
    "A protein sequence alone cannot scientifically determine exon–intron structure. "
    "BioProtein Studio therefore requires reference genomic/CDS information or a resolvable "
    "NCBI accession; it never invents a gene structure from protein sequence alone."
)
...
st.info(
    "Full automatic motif + spliced gene-structure analysis requires MEME Suite and EMBOSS. "
    "Use the supplied Conda environment on Linux/WSL for the complete local workflow."
)
